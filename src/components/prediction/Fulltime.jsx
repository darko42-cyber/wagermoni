...
const Fulltime = () => {
  ...
  const [unsettledCheck, setUnsetledCheck] =
    useState(false);
  ...
  const handleUnsettledChange = (e) => {
    ...
    if (e.target.checked) {
      let filteredData;
      filteredData = predictions.filter(
        (p) => p.gameType === "Fulltime"
      );
      filteredData = filteredData.filter(
        (d) => d.result === e.target.value
      );
      ...
      setfullTime(
        predictions.filter((p) => p.gameType === "Fulltime")
      );
    }
  };
  ...
  useEffect(() => {
    if (predictions) {
      let filteredData = predictions.filter(
        (p) => p.gameType === "Fulltime"
      );
      setfullTime(filteredData);
    }
